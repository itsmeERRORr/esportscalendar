const countryCodeMap: { [key: string]: string } = {
  ...
  // Adicione aqui quaisquer outros países ou variações de nomes que você precise
  'USA': 'US',
  'UK': 'GB',
  'England': 'GB',
  'Scotland': 'GB',
  ...
  'North Macedonia': 'MK',
  'Kosovo': 'XK', // Note que Kosovo não é universalmente reconhecido, mas tem um código não oficial
  'Online': 'WORLD',
  'TBA': 'WORLD',
  'TBD': 'WORLD'
}
...
export function getCountryCode(country: string): string {
  ...
  if (normalizedCountry === 'TBA' || normalizedCountry === 'TBD' || normalizedCountry === 'Online') {
    return 'WORLD'
  }
  const code = countryCodeMap[normalizedCountry]
  return code || 'WORLD' // Use 'WORLD' for unknown countries or online events
}

export function getCountryFlagUrl(country: string): string {
  const countryCode = getCountryCode(country)
  if (countryCode === 'WORLD') {
    return 'https://www.hltv.org/img/static/flags/30x20/WORLD.gif'
  }
